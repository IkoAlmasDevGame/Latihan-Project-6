        ...
        <?php
            ...
            if(!isset($_GET['page'])){
                ...
                switch ($_GET['page']) {
                    ...
                    case 'histori':
                        header("location:../histori/index.php?nama=".$_SESSION['nama_pengguna']);
                        break;
                        
                    case 'laporan':
                        header("location:../laporan/index.php?nama=".$_SESSION['nama_pengguna']);
                        break;
                    ...
                    case 'settings':
                        header("location:../settings/index.php?nama=".$_SESSION['nama_pengguna']);
                        break;

                    case 'keluar':
                        $date = Date("Y-m-d H:i:s a");
                        $row = $configs->prepare("UPDATE tb_pengguna SET created_At = ? WHERE email = ?");
                        $row->execute(array($date, $_SESSION['email_pengguna']));
                        $row2 = $configs->prepare("UPDATE tb_pengguna SET created_At = ? WHERE username = ?");
